...
void main() {
  ...
  group('Adversarial Scenarios & Stress Tests', () {
    testWidgets('1. Extreme Ultra-Wide 5K (5120x1440) Content Centering & MaxWidth Bounds',
      ...
      final renderBox = tester.renderObject(find.byKey(const Key('inner_content'))) as RenderBox;
    ...
    testWidgets('2. Extremely Narrow Viewport (240x600) Mobile Robustness',
      ...
      await tester.pumpWidget(
        MaterialApp(
          theme: AppTheme.lightTheme,
          home: const ResponsiveAppShell(
            child: Text('Narrow Screen Content'),
          ),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('3. Shallow Landscape Viewport (1024x300) Desktop TopNavBar Layout',
        (WidgetTester tester) async {
      ...
      final topNavBarBox = tester.renderObject(find.byType(TopNavBar)) as RenderBox;
      expect(topNavBarBox.size.height, equals(68.0));
      expect(find.text('Shallow Viewport Content'), findsOneWidget);
    });

    testWidgets('4. Standalone ResponsiveAppShell Without GoRouter (Vanilla Navigator)',
        (WidgetTester tester) async {
    ...
    testWidgets('5. GoRouter with Query Parameters & Deep Dynamic IDs Active Tab Resolution',
        (WidgetTester tester) async {
    ...
    testWidgets('6. Rapid Sequential Route Transitions on Desktop Without Stalling',
        (WidgetTester tester) async {
    ...
    testWidgets('7. TopNavBar Handles Safe Area Top Notch Padding on iPad / Tablet Viewports',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1024, 768);
      tester.view.devicePixelRatio = 1.0;
    ...
    testWidgets('8. Accessibility High Text Scaling (2.5x) Does Not Cause Flex Overflow on TopNavBar or App Shell',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(768.0, 900.0); // Exact minimum desktop width
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
    ...
    testWidgets('9. GoRouter Deep Path with Hash and Special Characters Tab Selection',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1280, 800);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      final router = GoRouter(
        ...
        routes: [
          ShellRoute(
            builder: (context, state, child) => ResponsiveAppShell(child: child),
            routes: [
              GoRoute(path: '/home', builder: (context, state) => const Text('Home Landing')),
              GoRoute(path: '/profile', builder: (context, state) => const Text('Profile Landing')),
            ],
          ),
        ],
      );
    ...
    testWidgets('12. Tablet upper boundary precision at 1024.0px and 1024.01px',
        (WidgetTester tester) async {
    ...
    testWidgets('13. ResponsiveAppShell custom backgroundColor override takes precedence in Light and Dark modes',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1024, 768);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
    ...
    testWidgets('14. Mobile shell hides BottomNavBar on auth and splash routes',
        (WidgetTester tester) async {
      tester.view.physicalSize = const Size(375, 667);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
        ...
        routes: [
          ShellRoute(
            ...
            routes: [
              ...
              GoRoute(path: '/register', builder: (context, state) => const Text('Register')),
              GoRoute(path: '/forgot-password', builder: (context, state) => const Text('Forgot Password')),
              GoRoute(path: '/splash', builder: (context, state) => const Text('Splash')),
            ],
          ),
        ],
      );
