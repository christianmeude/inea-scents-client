...
void main() {
  ...
  group('PageTransitionsTheme & CrossFadePageTransitionsBuilder Tests', () {
    test('AppTheme.lightTheme and darkTheme inject CrossFadePageTransitionsBuilder for desktop platforms', () {
      ...
      for (final platform in desktopPlatforms) {
        final lightBuilder = lightTheme.pageTransitionsTheme.builders[platform];
        expect(
          lightBuilder,
          isA<CrossFadePageTransitionsBuilder>(),
          reason: 'Expected lightTheme to use CrossFadePageTransitionsBuilder for $platform',
        );
        ...
        expect(
          darkBuilder,
          isA<CrossFadePageTransitionsBuilder>(),
          reason: 'Expected darkTheme to use CrossFadePageTransitionsBuilder for $platform',
        );
      }

      // Verify mobile platforms retain standard mobile builders
      expect(lightTheme.pageTransitionsTheme.builders[TargetPlatform.iOS], isA<CupertinoPageTransitionsBuilder>());
      expect(lightTheme.pageTransitionsTheme.builders[TargetPlatform.android], isA<ZoomPageTransitionsBuilder>());
    });

    testWidgets('CrossFadePageTransitionsBuilder builds FadeTransition widgets', (WidgetTester tester) async {
    ...
    testWidgets('Desktop navigation executes cross-fade transition on route push and pop (Windows)', (WidgetTester tester) async {
    ...
    testWidgets('Desktop navigation executes cross-fade transition on macOS in Dark Theme', (WidgetTester tester) async {
      final key = GlobalKey<NavigatorState>();
    ...
    testWidgets('Desktop navigation executes cross-fade transition on Linux and Fuchsia platforms', (WidgetTester tester) async {
    ...
    testWidgets('Mid-transition cancellation and reverse animation completes cleanly', (WidgetTester tester) async {
      final key = GlobalKey<NavigatorState>();
