...
void main() {
  ...
  group('Web Interactions Foundation - R1: Mouse Cursors', () {
    ...
    testWidgets('AppTheme button styles define SystemMouseCursors.click for enabled state', (tester) async {
      // Verify ElevatedButton theme mouse cursor
      final elevatedCursor = AppTheme.lightTheme.elevatedButtonTheme.style?.mouseCursor?.resolve({});
      expect(elevatedCursor, equals(SystemMouseCursors.click));

      final elevatedDisabledCursor = AppTheme.lightTheme.elevatedButtonTheme.style?.mouseCursor?.resolve({WidgetState.disabled});
      expect(elevatedDisabledCursor, equals(SystemMouseCursors.basic));

      // Verify OutlinedButton theme mouse cursor
      final outlinedCursor = AppTheme.lightTheme.outlinedButtonTheme.style?.mouseCursor?.resolve({});
      expect(outlinedCursor, equals(SystemMouseCursors.click));

      // Verify TextButton theme mouse cursor
      final textBtnCursor = AppTheme.lightTheme.textButtonTheme.style?.mouseCursor?.resolve({});
      expect(textBtnCursor, equals(SystemMouseCursors.click));

      // Verify IconButton theme mouse cursor
      final iconBtnCursor = AppTheme.lightTheme.iconButtonTheme.style?.mouseCursor?.resolve({});
      expect(iconBtnCursor, equals(SystemMouseCursors.click));
    });

    testWidgets('CustomTextField uses SystemMouseCursors.text and suffix button uses click', (tester) async {
  ...
  group('Web Interactions Foundation - R2: Hover States', () {
    testWidgets('PackageCard shifts background color and border on hover', (tester) async {
      const package = Package(
        id: 1,
        name: 'Deluxe Perfume Bar',
        description: 'Luxury scent experience',
    ...
    testWidgets('CustomTextField shifts background and border on hover', (tester) async {
      final controller = TextEditingController();
    ...
    testWidgets('AppTheme buttons define hover overlay/background shifts', (tester) async {
      // ElevatedButton hover background
      final elevatedHoverBg = AppTheme.lightTheme.elevatedButtonTheme.style?.backgroundColor?.resolve({WidgetState.hovered});
      expect(elevatedHoverBg, equals(const Color(0xFF5A3646)));

      // OutlinedButton hover background tint
      final outlinedHoverBg = AppTheme.lightTheme.outlinedButtonTheme.style?.backgroundColor?.resolve({WidgetState.hovered});
      expect(outlinedHoverBg, isNotNull);

      // TextButton hover background tint
      final textBtnHoverBg = AppTheme.lightTheme.textButtonTheme.style?.backgroundColor?.resolve({WidgetState.hovered});
      expect(textBtnHoverBg, isNotNull);
    });
  });

  group('Web Interactions Foundation - R3: Focus Rings & Keyboard Navigation', () {
    testWidgets('PackageCard shows visible focus ring and activates with Enter key', (tester) async {
      bool tapped = false;
      ...
      final action = detector.actions?[ActivateIntent] as CallbackAction<ActivateIntent>?;
      expect(action, isNotNull);
      action?.invoke(const ActivateIntent());
      expect(tapped, isTrue);
    });

    testWidgets('CustomTextField shows visible focus ring when focused', (tester) async {
      final controller = TextEditingController();
    ...
    testWidgets('AppTheme buttons render focus rings when focused', (tester) async {
      // Verify ElevatedButton focused side ring (2.5px)
      final elevatedStyle = AppTheme.lightTheme.elevatedButtonTheme.style;
      final elevatedFocusedSide = elevatedStyle?.side?.resolve({WidgetState.focused});
      ...
      final outlinedFocusedSide = outlinedStyle?.side?.resolve({WidgetState.focused});
      ...
      final textBtnFocusedSide = textBtnStyle?.side?.resolve({WidgetState.focused});
      ...
      final iconBtnFocusedSide = iconBtnStyle?.side?.resolve({WidgetState.focused});
    ...
    testWidgets('Dark theme button styles define visible focus rings and click cursors', (tester) async {
    ...
    testWidgets('PackageCard in dark theme renders dark background and light cream focus ring', (tester) async {
      const package = Package(
        id: 2,
    ...
    testWidgets('CustomTextField in dark theme shifts colors on hover and focus', (tester) async {
      final controller = TextEditingController();
